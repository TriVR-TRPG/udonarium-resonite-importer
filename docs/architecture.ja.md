# Udonarium Resonite Importer アーキテクチャ設計書

> このドキュメントはアーキテクチャの参照仕様です。設計原則・レイヤ構成・データ契約・実行フローを定義し、実装上の判断根拠を提供します。

## 1. 概要

CLI/GUI の両インターフェースから Udonarium ZIP を解析し、ResoniteLink 経由でインポートする。
本アーキテクチャは以下の目標のもとに設計されている。
...
### 方針
- **当面は現行の switch ベースを維持する**（Converter Registry 化は任意実施）。

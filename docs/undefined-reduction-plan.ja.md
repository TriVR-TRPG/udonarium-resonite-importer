...
- `npm run -s metrics:undefined:count`: 0
...
- フェーズ 1: 完了
- フェーズ 2: 完了
- フェーズ 3: 完了（本体コードの `undefined` 明示行は 0）
- フェーズ 4: 完了（ESLint ルールを `error` へ昇格）

実施済みの代表施策:

- `slotId !== undefined` などの比較を `!= null` に寄せた
- `func(..., undefined)` 呼び出しを回避（引数順整理・`null` 明示・シグネチャ整理）
- 戻り値注釈の `| undefined` を型推論または `null` ベースの `Maybe` へ統一
- `imageAspectRatioMap` / `imageAssetContext` / `ParserUtils` / `sharedMaterial` のホットスポットを優先改修
